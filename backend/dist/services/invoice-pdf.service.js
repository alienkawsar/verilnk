...
const toLineValue = (value) => (value && value.trim() ? value.trim() : null);
const getInvoiceStatusTheme = (status) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        const normalizedLineItems = (input.lineItems && input.lineItems.length > 0)
            ? input.lineItems.map((item) => ({
                ...
                totalCents: Math.max(0, Math.floor(typeof item.totalCents === 'number'
                    ? item.totalCents
        ...
        const subtotalCents = Math.max(0, Math.floor(typeof input.subtotalCents === 'number'
            ? input.subtotalCents
