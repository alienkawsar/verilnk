"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getConfiguredPaymentProvider = exports.validatePaymentConfiguration = exports.getSslcommerzSandboxMode = exports.PaymentConfigurationError = void 0;
const PAYMENT_PROVIDER_VALUES = [
    'stripe',
    'sslcommerz',
];
class PaymentConfigurationError extends Error {
    constructor(message) {
        super(message);
        this.name = 'PaymentConfigurationError';
    }
}
exports.PaymentConfigurationError = PaymentConfigurationError;
const hasValue = (value) => Boolean(value && value.trim().length > 0);
const getSslcommerzSandboxMode = () => {
    const value = process.env.SSLCOMMERZ_IS_SANDBOX;
    if (!value)
        return false;
    return ['true', '1', 'yes'].includes(value.trim().toLowerCase());
};
exports.getSslcommerzSandboxMode = getSslcommerzSandboxMode;
const isValidBooleanToken = (value) => {
    const normalized = value.trim().toLowerCase();
    return (normalized === 'true' ||
        normalized === 'false' ||
        normalized === '1' ||
        normalized === '0' ||
        normalized === 'yes' ||
        normalized === 'no');
};
const resolvePaymentProvider = () => {
    const rawProvider = String(process.env.PAYMENT_PROVIDER || '')
        .trim()
        .toLowerCase();
    if (!PAYMENT_PROVIDER_VALUES.includes(rawProvider)) {
        throw new PaymentConfigurationError('PAYMENT_PROVIDER must be set to "stripe" or "sslcommerz".');
    }
    return rawProvider;
};
const requireEnvironmentVariables = (provider, envNames) => {
    const missing = envNames.filter((name) => !hasValue(process.env[name]));
    if (missing.length > 0) {
        throw new PaymentConfigurationError(`Missing required environment variables for PAYMENT_PROVIDER=${provider}: ${missing.join(', ')}`);
    }
};
let cachedProvider = null;
const validatePaymentConfiguration = () => {
    const isProduction = process.env.NODE_ENV === 'production';
    const provider = resolvePaymentProvider();
    requireEnvironmentVariables(provider, ['APP_URL']);
    if (provider === 'stripe') {
        requireEnvironmentVariables(provider, ['STRIPE_SECRET_KEY']);
        if (isProduction) {
            requireEnvironmentVariables(provider, ['STRIPE_WEBHOOK_SECRET']);
        }
    }
    if (provider === 'sslcommerz') {
        requireEnvironmentVariables(provider, [
            'SSLCOMMERZ_STORE_ID',
            'SSLCOMMERZ_STORE_PASSWORD',
        ]);
        const sandboxValue = process.env.SSLCOMMERZ_IS_SANDBOX;
        if (hasValue(sandboxValue) &&
            !isValidBooleanToken(sandboxValue)) {
            throw new PaymentConfigurationError('SSLCOMMERZ_IS_SANDBOX must be a boolean token: true/false/1/0/yes/no.');
        }
    }
    return provider;
};
exports.validatePaymentConfiguration = validatePaymentConfiguration;
const getConfiguredPaymentProvider = () => {
    if (cachedProvider) {
        return cachedProvider;
    }
    cachedProvider = (0, exports.validatePaymentConfiguration)();
    return cachedProvider;
};
exports.getConfiguredPaymentProvider = getConfiguredPaymentProvider;
