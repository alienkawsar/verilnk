...
const upsertInvoiceMetadata = async (invoice: Invoice, patch: Record<string, unknown>) => {
    const merged = {
...
const resolveDurationFromInvoiceMetadata = (metadata: Record<string, unknown>): number => {
...
const markAttemptFailed = async (params: {
    ...
    const status = params.status === PaymentAttemptStatus.CANCELED
...
const createStripeCheckout = async (params: {
    organization: { id: string; name: string; email: string; address: string; phone: string };
    ...
    if (
        ...
        if (event.type === 'checkout.session.async_payment_failed' || event.type === 'checkout.session.expired') {
            await markAttemptFailed({
                paymentAttemptId,
                gateway: BillingGateway.STRIPE,
                status: event.type === 'checkout.session.expired' ? PaymentAttemptStatus.CANCELED : PaymentAttemptStatus.FAILED,
...
export const processSSLCommerzCallback = async (params: SSLCommerzCallbackParams) => {
